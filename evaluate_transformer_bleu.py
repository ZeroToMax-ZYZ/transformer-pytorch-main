...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate Transformer BLEU with checkpoint averaging.")
    ...
    parser.add_argument("--src-path", type=str, default="data/wmt14_bpe_en_de/test.en", help="BPE source text path.")
    parser.add_argument("--tgt-path", type=str, default="data/wmt14_bpe_en_de/test.de", help="BPE reference text path.")
    ...
    parser.add_argument("--max-output-extra-len", type=int, default=50, help="Decode up to src_len + this extra length.")
...
def resolve_checkpoint_paths(args: argparse.Namespace, experiment_dir: Path) -> List[Path]:
    if args.checkpoint_paths:
        return [Path(p) for p in args.checkpoint_paths]

    checkpoint_dir = experiment_dir / "checkpoints"
    periodic_paths = list_periodic_checkpoints(checkpoint_dir)
    return periodic_paths[-args.average_last_n :]
